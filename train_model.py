...
# --- SENSOR METADATA ---
SENSOR_LABELS = {
    's2':  'T24 - Fan Inlet Temperature (deg R)',
    's3':  'T30 - LPC Outlet Temperature (deg R)',
    's4':  'T50 - HPC Outlet Temperature (deg R)',
    's6':  'P30 - Total Hydraulic Pressure (psia)',
    's7':  'Nf  - Physical Fan Speed (rpm)',
    's8':  'Nc  - Physical Core Speed (rpm)',
...
fleet_temporal_std  = X_train[temporal_std_cols].mean().to_dict()
...
# --- TRAINING ---
print(f"\n[3/6] Training Diverse Ensemble...")
models = []
model_metadata_list = []

for i, cfg in enumerate(ensemble_configs):
    print(f"  Node {i+1}/5 - {cfg['name']}")
    m = XGBRegressor(**cfg['params'])
    m.fit(X_train, y_train, eval_set=[(X_test, rul_actual['RUL'])], verbose=False)
    models.append(m)
...
metadata = {
    'training_summary': {
        ...
        'train_engines':     int(train['RUL'].shape[0]),
